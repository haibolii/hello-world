...
A stationary kernel function is a function whose value only depends on the difference between $\vec x$ and $\vec x'$. We write $k$ to take only single argument $\vec r = \vec x - \vec x'$. The mostly commonly used stationary kernel function is the \textit{squared exponential}(SE) kernel function, which has the form
\begin{equation}
